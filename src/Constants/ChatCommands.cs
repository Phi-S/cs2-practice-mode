namespace Cs2PracticeMode.Constants;

public static class ChatCommands
{
    public const string Alias = "alias";
    public const string RemoveAlias = "removealias";
    public const string GlobalAlias = "globalalias";
    public const string RemoveGlobalAlias = "removeglobalalias";

    public const string Bot = "bot";
    public const string TBot = "tbot";
    public const string CtBot = "ctbot";
    public const string CrouchBot = "cbot";
    public const string BoostOnBot = "boost";
    public const string CrouchBoostOnBot = "cboost";
    public const string SwapWithBot = "swapbot";
    public const string MoveBot = "movebot";
    public const string NoBot = "nobot";
    public const string NoBots = "nobots";
    public const string ClearBots = "clearbots";

    public const string BreakProps = "break";

    public const string ChangeMap = "map";

    public const string ClearPlayerGrenades = "clear";
    public const string ClearAllGrenades = "clearall";

    public const string Help = "help";

    public const string Countdown = "countdown";

    public const string RconLogin = "rconlogin";
    public const string Rcon = "rcon";

    public const string FlashMode = "flash";
    public const string StopFlashMode = "stop";

    public const string GrenadeMenu = "nades";

    public const string SaveGrenade = "save";
    public const string DeleteGrenade = "delete";

    public const string SelectGrenade = "select";
    public const string FindGrenade = "find";
    public const string RenameGrenade = "rename";
    public const string GrenadeDescription = "desc";
    public const string AddTagToGrenade = "addtag";
    public const string RemoveGrenadeTag = "removetag";
    public const string ShowGrenadeTags = "tags";
    public const string DeleteGrenadeTag = "deletetag";
    public const string ClearGrenadeTags = "cleartags";

    public const string Throw = "throw";
    public const string Rethrow = "rethrow";
    public const string Last = "last";
    public const string Forward = "forward";
    public const string Back = "back";

    public const string NoFlash = "noflash";

    public const string Settings = "settings";

    public const string Spawn = "spawn";
    public const string TSpawn = "tspawn";
    public const string CtSpawn = "ctspawn";
    public const string BestSpawn = "bestspawn";
    public const string WorstSpawn = "worstspawn";

    public const string SwapTeam = "swap";

    public const string Timer = "timer";
    public const string Timer2 = "timer2";
}