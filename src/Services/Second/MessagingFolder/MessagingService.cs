...
namespace Cs2PracticeMode.Services.Second.MessagingFolder;
...
public class MessagingService : Base
{
    ...
    public MessagingService(PluginConfigService pluginConfigService) : base(LoadOrder.Second)
    {
    ...
    {
        foreach (var openHtmlMenu in _openHtmlMenus) MenuManager.CloseActiveMenu(openHtmlMenu.Key);
